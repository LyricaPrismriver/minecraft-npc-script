...
function meleeAttack(event)
{
    ...
    if(data.has("double_hit"))
    {
        var double_hit_count = data.get("double_hit");
        if(double_hit_count >= 1)
        {
            data.remove("double_hit");
            melee.setDelay(atk_speed);
        }
        else
            data.put("double_hit", double_hit_count+=1);

    }else
    {
        data.put("double_hit", 0)
        melee.setDelay(2);
    }
    
    if(data.has("no_attack"))
...
function damaged(event){
	...
	if(source)
	{
        if(source.getType() == 1)
        {
            ...
            //如果有格挡标记则记住目标
            if(data.has("yinian_block"))
            {
                npc.executeCommand("/playsound se.sword_hit music @p");
                data.put("yinian_block_target", source);
                data.remove("yinian_block");
                //控制目标
...
function tick(event)
{
    ...
    if(npc.isAlive())
    {
        ...
        if(ct > count_ticks)
        {
            ...
            if(data.has("xianshi_cooldown"))
                data.remove("xianshi_cooldown")
            if(data.has("yinghua_cooldown"))
                data.remove("yinghua_cooldown")
            if(data.has("yinian_cooldown"))
                data.remove("yinian_cooldown")
            if(data.has("is_busy"))
                data.remove("is_busy")
            
            data.put("damage_score", new Object);
        }
        //释放符卡
        if(!data.has("is_busy"))
        {
        ...
        var health = event.npc.getHealth();
        if(health > 700)
            atk_speed = 30;
        else if(health > 400)
            atk_speed = 20;
...
function interact(event)
{
    ...
    {
        event.npc.say(player_name + "造成" + Math.ceil(damage_score[player_name]) + "点伤害");
    }
...
function yinghua_event(event)
{
    ...
    if(data.has("yinghua_cooldown"))
    {//计算冷却的地方
        var cd = data.get("yinghua_cooldown");
        if(cd <= world.getTotalTime())
            data.remove("yinghua_cooldown");
    }
    if(!data.has("yinghua_cooldown")&&target&& npc.isAttacking()&&!data.has("is_busy"))
    {
        data.put("is_busy", 1);//NPC忙标志
        ...
        data.put("yinghua_cooldown", world.getTotalTime() + yinghua_cooldown * 20);//冷却标志
...
function yinian_event(event)
{
    ...
    if(data.has("yinian_cooldown"))
    {//计算冷却的地方
        var cd = data.get("yinian_cooldown");
        if(cd <= world.getTotalTime())
            data.remove("yinian_cooldown");
    }
    if(!data.has("yinian_cooldown")&&target&& npc.isAttacking()&&!data.has("is_busy"))
    {
        data.put("is_busy", 1);//NPC忙标志
        var x = npc.getX(),y = npc.getY(),z = npc.getZ();
        var players = [];
        ...
        var MyThread_yinian = Java.extend(Thread, { 
            run: function() { 
                ...
                data.put("yinian_block", 1);
                data.put("no_attack", 1);

                Thread.sleep(yinian_block_duration);

                if(data.has("yinian_block_target"))
                {
                    var target = data.get("yinian_block_target");
                    ...
                    data.remove("yinian_block_target");
                }
                //清除无敌标记
                if(data.has("Invincible"))
                    data.remove("Invincible");
                //清除禁止攻击标记
                if(data.has("no_attack"))
                    data.remove("no_attack");
                //清除格挡标记
                if(data.has("yinian_block"))
                    data.remove("yinian_block");
        ...
        data.put("yinian_cooldown", world.getTotalTime() + yinian_cooldown * 20);//冷却标志
        //event.setCancelled(true);
    }
}
//现世斩
function xianshi_event(event)
{
    ...
    if(data.has("xianshi_cooldown"))
    {//计算冷却的地方
        var cd = data.get("xianshi_cooldown");
        if(cd <= world.getTotalTime())
            data.remove("xianshi_cooldown");
    }
    if(!data.has("xianshi_cooldown")&&target&& npc.isAttacking()&&!data.has("is_busy"))
    {
        data.put("is_busy", 1);//NPC忙标志
        var x = npc.getX(),y = npc.getY(),z = npc.getZ();
        var x2 = target.getX(),y2 = target.getY(),z2 = target.getZ();
        ...
        data.put("xianshi_cooldown", world.getTotalTime() + xianshi_cooldown * 20);//冷却标志
        //event.setCancelled(true);
    }
}
...
function chengfo_event(event)
{
    ...
    if(data.has("chengfo_cooldown"))
    {//计算冷却的地方
        var cd = data.get("chengfo_cooldown");
        if(cd <= world.getTotalTime())
            data.remove("chengfo_cooldown");
    }
    if(!data.has("chengfo_cooldown")&&target&& npc.isAttacking()&&!data.has("is_busy"))
    {
        data.put("is_busy", 1);//NPC忙标志
        //var in_r = 3.5;//内径
        //var out_r = 5.2;//外径
        ...
        data.put("chengfo_cooldown", world.getTotalTime() + chengfo_cooldown * 20);//冷却标志
        //event.setCancelled(true);
    }
}
...
function miari_event(event)
{
    ...
    if(data.has("mirai_cooldown"))
    {//计算冷却的地方
        var cd = data.get("mirai_cooldown");
        if(cd <= world.getTotalTime())
            data.remove("mirai_cooldown");
    }
    if(!data.has("mirai_cooldown")&&target&& npc.isAttacking()&&!data.has("is_busy"))
    {
        data.put("is_busy", 1);//NPC忙标志
        var mr = 3.5;
        var mr2 = 5.2;
        ...
        data.put("mirai_cooldown", world.getTotalTime() + mirai_cooldown * 20);//冷却标志
        //event.setCancelled(true);
    }
}
...
function doParticleDamage(entities, damage, particle_count, event, hitsound)
{
    ...
    {
        if(hitsound)
            npc.executeCommand("/playsound "+ hitsound +" music @p");
...
function safeFindEntities(x, y, z, range, type, event)
{
    ...
    var world = npc.getWorld();
    var entities = [];
    var raw_entities = world.getNearbyEntities(x, y, z, range, type);
